...
export const register = async (req, res) => {
  ...
  res.status(201).send("Utente registrato").end();
};

export const login = async (req, res) => {
  try {
    const user = Object.values(utenti).find(
      (usr) => usr.email === req.body.email
    );
    if (user) {
      ...
      if (isPasswordValid) {
        ...
        const obj = {
          user: Object.keys(utenti).find((key) => utenti[key] === user),
          token: token,
        };
        res.send(obj);
      } else {
